 ...
 */
export type WalletSection = "events" | "listings" | "giving" | "profile";
 ...
 * (`/wallet/my-tickets/event/:uuid`) stay on Tickets; anything else falls back to
 * Tickets so the wallet always has a section to render.
 */
...
/** Any wallet section route, including ticket detail pages. */
export function isWalletSectionPath(pathname = "") {
  const path = pathOnly(pathname);
  return WALLET_SECTION_PREFIXES.some(
